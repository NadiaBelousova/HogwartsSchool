...
class FacultyServiceTest {
    ...
    void findFacultyByIdCorrectly() {
        long id = 2L;
        Mockito.when(repository.findById(id)).thenReturn(Optional.ofNullable(FACULTY_2));
        assertEquals(Optional.of(FACULTY_2), out.findFaculty(FACULTY_2.getId()));
    ...
    void findByColor() {
        ...
        Mockito.when(repository.getByNameIgnoreCaseOrColorIgnoreCase(color, name)).thenReturn(Optional.ofNullable(FACULTY_1));
        assertEquals(Optional.of(FACULTY_1), out.findByNameOrColor(color, name));
